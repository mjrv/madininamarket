...
class UploadHandler
{
	...
	public function uploadFile($entity, $property, $annotation)
	{
		...
		if($file instanceOf UploadedFile) 
		{
			...
			$this->accessor->setValue($entity, $annotation->getFilename(), $annotation->getPath().'/'.$filename);
			// $this->accessor->setValue($entity, $annotation->getUpdateAt(), new \DateTime());
		}
	}
	...
	private function getFileFromFilename($entity, $annotation)
	{
		...
		// else return new File($annotation->getPath(). DIRECTORY_SEPARATOR .$filename);
	}

	public function removeFile($entity, $property)
	{
		$file = $this->accessor->getValue($entity, $property);
		if ($file instanceOf File) @unlink($file->getRealPath());
	}
}